...
void MessageHandler::send_byte(Connection& conn, int code) {
  try {
...
void MessageHandler::send_code(Connection& conn, int code) {
  send_byte(conn, code);
}

void MessageHandler::send_int(Connection& conn, int value) {
...
void MessageHandler::send_int_parameter(Connection& conn, int param) {
  send_code(conn, Protocol::PAR_NUM);
  send_int(conn, param);
}

void MessageHandler::send_string_parameter(Connection& conn, const string& param) {
...
unsigned char MessageHandler::recv_byte(Connection& conn) {
  int byte;
  try {
    byte = conn.read();
  } catch(...) {
...
int MessageHandler::recv_code(Connection& conn) {
  return recv_byte(conn);
}

int MessageHandler::recv_int(Connection& conn) {
...
int MessageHandler::recv_int_parameter(Connection& conn) {
...
std::string MessageHandler::recv_string_paramter(Connection& conn) {
  int code = recv_code(conn);
